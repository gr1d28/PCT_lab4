...
void move_particles_line(struct particle *p, struct particle *f, struct particle *v, float *m, int n,
double dt)
{
  for (int i = 0; i < n; i++)
  {
...
void func_line(struct particle *p, struct particle *f, struct particle *v, float *m, int n, double dt, double *tforces, double *tmove)
{
  for (double t = 0; t <= 1; t += dt) // Цикл по времени (модельному)
  {
    ...
    *tforces += wtime();
    *tmove -= wtime();
    move_particles_line(p, f, v, m, n, dt); // Перемещение тел O(N)
    *tmove += wtime();
  }
}
...
{
  FILE *fp = fopen("add_memory.dat", "w");
  double boost = 0;
  ...
  struct particle *f_cp[omp_get_max_threads()];
  ...
  for(int i = 0; i < omp_get_max_threads(); i++)
  {
    ...
    f_cp[i] = malloc(sizeof(struct particle) * n);
  }
  ...
  t_line = wtime();
  func_line(p, f[0], v, m, n, dt, &tforces, &tmove);
  t_line = wtime() - t_line;
